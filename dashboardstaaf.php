...
<body div class="p-3 mb-2 bg-info text-white">
  ...
  <h1 class = "x">Toon alle data van een brug.</h1>
...
foreach ($result as $row)
{
  ...
  $tabel = $row['tabelnaam'];
  $brugnaam = $row['brugnaam'];
  $button = "<form id=\"tabel\" name=\"tabel\" class =\"brug$n\" action=\"kolomdiagramalles.php\" method=\"post\">
  <input id = \"detabel\" type = \"hidden\" name = \"detabel\" value = \"$tabel\">
  <input id = \"brugnaam\" type = \"hidden\" name = \"brugnaam\" value = \"$brugnaam\">
  <input class=\"btn btn-primary\" id = \"knop\" type = \"button\" onclick = \"submit()\" value = \"$brugnaam\"> </form>";
  echo "<h4>Staafdiagram van:</h4>" . $button;
}
